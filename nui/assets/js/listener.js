  ...
  window.addEventListener('message', function (event) {
    ...
    if (action == 'setStamina') {
      if (data) {
        ...
        Circle.StaminaIndicator.path.setAttribute('stroke', stamina < 0.1 ? 'rgb(255, 0, 0)' : 'rgb(255, 0, 255)');
    ...
    if (action == 'setOxygen') {
     ...
     if (action == 'setVehicle') {
       if (data) {
         ...
         percSpeed < 0.01 && (SpeedIcon.textContent = 'KRS® │Development');

         Fuel.style.display = fuel !== false ? 'block' : 'none';
         fuel <= 0.15 && FuelIcon.classList.toggle('flash');
         Circle.FuelIndicator.path.setAttribute('stroke', fuel > 0.1 ? 'rgb(200, 200, 200)' : 'rgb(0, 130, 255)');
    ...
    if (action == 'setSeatbelt') {
      ...
      Seatbelt.style.color = data.buckled ? 'rgb(0, 255, 100)' : 'rgb(255, 0, 100)';
    }
